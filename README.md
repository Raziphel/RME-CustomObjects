...
Primitive blocks can move without a custom Unity AssetBundle: set **Move Offset**, optional **Rotate By**, and **Motion Duration**. PingPong travels out and back; Loop restarts at the initial transform. This is server-driven, so moving collidable primitives retain their authoritative collider. For full Unity animation, assign an **Animator Controller** and an **Animator Bundle** filename. The normal JSON export builds the named server-platform AssetBundle beside the JSON; copy both files into the same server custom-object folder. Do not combine the two motion methods on one primitive.

To author a controller animation, create an Animator Controller asset in Unity, add clips that animate the selected primitive's transform, then assign that asset and a simple bundle filename (for example, `moving-door`) in the primitive Inspector. In the builder select the server platform, export the custom object, and copy the generated `moving-door` bundle alongside its JSON. All primitives using the same bundle name must reference that same controller.
